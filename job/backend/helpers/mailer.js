...
exports.sendVerificationEmail = async (email, name, url) => {
  ...
  try {
    ...
    const stmp = await nodemailer.createTransport({
      service: "gmail",
      auth: {
        type: "OAuth2",
        user: EMAIL,
    ...
    const mailOptions = {
      ...
      html: `<div style="max-width:700px;margin-bottom:1rem;display:flex;align-items:center;gap:10px;font-family:Roboto;font-weight:600;color:#3b5998"><span>Action requise : Activate your  account</span></div><div style="padding:1rem 0;border-top:1px solid #e5e5e5;border-bottom:1px solid #e5e5e5;color:#141823;font-size:17px;font-family:Roboto"><span>Hello ${name}</span><div style="padding:20px 0"><span style="padding:1.5rem 0">You recently created an account . To complete your registration, please confirm your account.</span></div><a href=${url} style="width:200px;padding:10px 15px;background:#4c649b;color:#fff;text-decoration:none;font-weight:600">Confirm your account</a><br><div style="padding-top:20px"><span style="margin:1.5rem 0;color:#898f9c"> </span></div></div>`,
    };
    stmp.sendMail(mailOptions, (err, res) => {
      if (err) console.log(err, "mail js if");
      return res;
    });
  } catch (err) {
    console.log(err, "mail js");
  }
};
